...
def _init(globe, readDistance = True, MaxStep = 81, filename="Data_for_Test.csv"):
...
def EH(tau, lamda):
	...
	E_per_step = eta * power_total * g_BR

	return E_t, E_per_step
...
def env_state(step, tau, lamda):
	...
	reward, energy_per_step = EH(tau, lamda)
	SNR = capacity (d_ru[step], tau, lamda)
	if SNR < 12:
		reward = 0
	radio_state = np.array([next_dru])

	return reward, radio_state, energy_per_step

def Grid_step(action, step):
	t = globe.get_value('t')
	tau = action[0]
	lamda = action[1]
	reward, radio_state, energy_per_step = env_state(step, tau, lamda)
	done = False
	if step == t - 1:
		done = True

	return reward, done, energy_per_step 
